...
/// runs.
pub fn rle_from_dense<T, F>(values: Vec<T>, mut same: F) -> (Vec<T>, Vec<(Range<usize>, u32)>)
where
    F: FnMut(&T, &T) -> bool,
{
    if values.is_empty() {
    ...
    // Consume `values` so we can move (not clone) elements into the table.
    ...
    table.push(first); // move, not clone
    ...
    for (i, v) in iter.enumerate() {
        ...
        if !same(&v, &table[cur_id as usize]) {
            runs.push((start..idx, cur_id));

            // Start a new run
            start = idx;
            table.push(v); // move, not clone
...
pub fn rle_from_value_runs<T: Clone + PartialEq>(
    ...
    let mut push_run = |range: Range<usize>, v: &T| {
        ...
        if let Some((last_r, last_id)) = out.last_mut()
            && last_r.end == range.start && *last_id == id {
...
pub(crate) fn merge_value_runs<T: Eq + Clone>(
    ...
    let mut append = |range: Range<usize>, value: T| {
        if let Some((last_r, last_v)) = out.last_mut()
            && last_r.end == range.start && *last_v == value {
                last_r.end = range.end;
                return;
            }
...
mod tests {
    ...
    fn multi_overlap_mixed_values() {
        ...
        assert_eq!(
            out,
            vec![(0..3, 1), (3..7, 2), (7..10, 1), (10..12, 3), (12..20, 4)]
        );
    }
